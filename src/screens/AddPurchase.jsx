...
import { getItems, addItem, addPurchase } from '../database'
...
function AddPurchase() {
  ...
  const getTotal = () => billItems.reduce((sum, b) => sum + b.price * b.qty, 0)
  ...
  const handleSave = async () => {
    ...
    setLoading(true)
    await addPurchase(supplierName.trim(), billItems, getTotal())
    setLoading(false)
    setSuccess(true)
  ...
  return (
    <div className="screen page-enter">
      {success && <SuccessCheck message="Purchase Saved!" />}
      ...
      <div className="screen-body">
        <div className="form-card">
          <div className="card-label">SUPPLIER</div>
          <input className="input-field" placeholder="Type supplier name" value={supplierName}
        ...
        {billItems.length > 0 && (
          <div className="bill-total">
            <div className="total-row"><span>Items</span><span>{billItems.length}</span></div>
            <div className="total-row"><span>Quantity</span><span>{billItems.reduce((s, b) => s + b.qty, 0)}</span></div>
        ...
        <div className="bill-actions">
          ...
          <button className="btn-green pressable" onClick={handleSave} disabled={loading}>
            {loading ? <span className="btn-loading"><span className="spinner" /> Saving...</span> : '✓ Save Purchase'}
          </button>
        </div>
      </div>
    </div>
