  ...
  return (
    <div className="success-overlay">
      <div className="success-box">
        ...
        <p className="success-text">Bill Saved!</p>
...
function GenerateBill() {
  ...
  const handleSave = async (print = false) => {
    ...
    await addSale(customer.id, customer.name, billItems, getSubtotal(), getDiscount(), getTotal(), 'bill')
  ...
  const handlePrint = () => {
    const lines = billItems.map(b => `${b.name.padEnd(22)} x${b.qty}   Rs ${(b.price * b.qty).toLocaleString()}`).join('\n')
    const content = `
╔══════════════════════════════╗
         LEDGR RECEIPT
╚══════════════════════════════╝
...
Discount : Rs ${getDiscount().toLocaleString()}
──────────────────────────────
...
══════════════════════════════
    ...
    win.document.write(`<pre style="font-size:13px;padding:24px;font-family:'Courier New',monospace;line-height:1.6;">${content}</pre>`)
    ...
    <div className="screen page-enter">
      ...
      <div className="screen-body">
        ...
        <div className="bill-actions">
          <button className="btn-outline pressable" onClick={() => handleSave(true)} disabled={loading}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
              <IconPrinter size={16} /> Print Bill
            </span>
          </button>
          <button className="btn-green pressable" onClick={() => handleSave(false)} disabled={loading}>
            {loading ? <span className="btn-loading"><span className="spinner" /> Saving...</span> : '✓ Save Bill'}
          </button>
        </div>
      </div>
    </div>
